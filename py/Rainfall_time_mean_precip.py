...
#--------DJF season--------- 
ncfile0 = Dataset(path + '/CRU_Africa_ts4.05.1901.2020.pre.nc', 'r', format='NETCDF4')
...
pr_djf = np.mean(pr_djf,axis=0) #timmean 
#pr_djf = np.mean(pr_djf,axis=[1,2])  #fldmean
print(pr_djf.max(), pr_djf.min())

#-------mask precipitation below 1 mm/day
#pr_djf = ma.masked_where(pr_djf <= 30.0, pr_djf)

#=========================Map resources==============================
fig = plt.figure(figsize=(7.40,6.10))
kwargs = {'format': '%.1f'}  # to fix decimals at X numbers after - put **kwargs in plt.cbar 
[lon2d, lat2d] = np.meshgrid(lon, lat)


prj = ccrs.PlateCarree(central_longitude=0.0)
#levels = np.linspace(1., 200.,11)
...
cs1 = plt.contourf(lon2d,lat2d,pr_djf,levels = np.arange(0., 150.,10), cmap=plt.cm.rainbow)
#cs1=plt.pcolormesh(lon2d,lat2d,pr_djf)
axa.set_extent([-9 ,-1, 3, 12])
axa.set_xticks(range(-9,-1,1), crs=prj)
axa.set_yticks(range(3,12,1), crs=prj)
...
cb0.set_label('[mm/month]',rotation=270, labelpad=15) 

plt.show()

#save('/home/students-aimssn/Documents/climate_lab/figures/Rainfall_time_mean', ext='png', close=True, verbose=True)  # save high quality figures
